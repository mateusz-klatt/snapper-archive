...
# transaction; missing or default-routed days are never purge targets.
set -euo pipefail
...
case "$PARENT_RELKIND" in
  r)
    echo "REFUSE $DAY: $SCHEMA.trades has no daily partitions" >&2
    exit 1
    ;;
  p)
    [[ "$PARTITION_KEY_OK" == "t" ]] || {
...
case "$PARTITION_STATE" in
  PARTITION)
    ;;
  DEFAULT)
    echo "REFUSE $DAY: no exact trades partition (already purged, not yet created, or outside the partitioned range); default partition $TARGET_PARTITION_SCHEMA.$TARGET_PARTITION_NAME is not a purge target" >&2
    exit 1
    ;;
  MISSING)
    echo "REFUSE $DAY: no exact trades partition (already purged, not yet created, or outside the partitioned range)" >&2
    exit 1
    ;;
  *)
